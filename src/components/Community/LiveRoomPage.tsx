...
const LiveRoomPage: React.FC = () => {
  ...
  const resolveDisplayName = (from: string) => {
    ...
    if (!u) {
      // fallback: nếu chưa có trong roster thì trả lại id thô
      return from;
    }
    // Nếu là chính mình → ưu tiên hiển thị "Bạn"
    return u.isSelf ? "Bạn" : u.userName;
  };
...
>
  ...
  <div className="px-4 py-2 bg-yellow-50 border-b border-yellow-200 flex items-start gap-3">
    ...
    <div className="flex-1">
      ...
      <div className="text-[11px] text-gray-400">
        {resolveDisplayName(shownPinned.from)} •{" "}
  ...
  const isMe =
    sender?.isSelf ?? String(m.from) === String(grant?.rtcUid);
  ...
  const displayName = sender
    ...
    : isMe
    ? "Bạn"
    : m.from; // fallback nếu chưa tìm thấy trong roster
